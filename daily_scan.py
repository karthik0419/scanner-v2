...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  DAILY MORNING SCAN  —  {today}")
    print(f"{'='*70}")
    ...
    if args.sector:
        ...
        if not hot_sectors and sector_perf:
            hot_sectors = [sector_perf[0][0]]  # even if down, pick least bad
    ...
    sector_syms = list(dict.fromkeys(sector_syms))  # deduplicate
